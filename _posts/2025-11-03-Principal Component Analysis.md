...
To illustrate the asymptotic distributions, let's do a Monte Carlo simulation on a 2-dimensional probability distribution. We take a bivariate t-distribution, that is $$ X = \frac{Z}{\sqrt{W / \nu}}, \hspace{0.5cm} Z \sim \mathcal{N} (0, \Sigma), \hspace{0.5cm}, W \sim \chi_\nu^2, $$ where $\nu$ denotes the degrees of freedom. Set $$ \Sigma = \begin{pmatrix} 1 & 0.8 \\ 0.8 & 1 \end{pmatrix} $$ and $\nu = 5$. The two eigenvalues of $\Sigma$ are $\lambda_1^{(\Sigma)} = 1.8$ and $\lambda_2^{(\Sigma)} = 0.2$. However, we can calculate that $Var(X) = \frac{\nu}{\nu - 2} \Sigma$ and so $\lambda_1 = 5/3 * 1.8 = 3$ and $\lambda_2 = 5/3 * 0.2 = 1/3$. For $l_i$, the estimate of $\lambda_i$, we have asymptotically that $\sigma_i^2 = Var((\beta_i^2\prime X)^2)$. Doing some cumbersome math leads to $Y_i = \beta_i X$, $Y_i^2 \sim \lambda_i \cdot F{1, \nu}$ so $$ \sigma_i^2 = Var(Y_i^2) = \lambda_i^2 \cdot \frac{2\nu^2 (\nu - 1)}{(\nu - 2)^2(\nu - 4)}. $$ Below, we illustrate the asymptotic distribution of the sample eigenvalues, and overlay the theoretical asymptotic normal approximation.

![PCA asymptotics](../assets/images/PCA_asymptotics.png)